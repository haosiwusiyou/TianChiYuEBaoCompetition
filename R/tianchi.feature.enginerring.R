...
feature.month <- function(date.range, user.balance.data,   interval = 7 ){
  # browser()
  ...
  user.balance.data[, 2] <- (user.balance.data[, 2]-min(user.balance.data[, 2]))/(max(user.balance.data[, 2])-min(user.balance.data[, 2])) + 1
  user.balance.data[, 3] <- (user.balance.data[, 3]-min(user.balance.data[, 3]))/(max(user.balance.data[, 3])-min(user.balance.data[, 3])) + 1
  ...
  for( i in col.pos ){
    month.purchase.data[ col.pos == i ,'month.purchase'] <- sum(user.balance.data[ col.pos == i , 2] ) / sum( user.balance.data[, 2] )
    month.redeem.data[ col.pos == i ,'month.redeem'] <- sum(user.balance.data[ col.pos == i , 3] ) / sum( user.balance.data[, 3] )
  }
...
feature.history <- function(date.range, user.balance.data, date.max.withdraw = 0, week.max.withdraw = c(4) ){
    ...
    if( date.max.withdraw > 0){
      for( i in 1:date.max.withdraw){
        ...
        for( j in 1:length(sel.dates)){
          history.purchase[j, history.att.date[i]] <- user.balance.data[ user.balance.data[,1] %in% sel.dates[j] , 2]
          history.redeem[j, history.att.date[i]] <- user.balance.data[  user.balance.data[,1] %in% sel.dates[j], 3]
        }
      }
    }
    # browser()
    if( length(week.max.withdraw) > 0){
      ...
      for( i in 1:length(week.max.withdraw)){
          history.purchase[, history.att.week[1]] <- history.purchase[, history.att.week[1]] +  user.balance.data[ user.balance.data[,1]  %in% (date.range - ( 7*(week.max.withdraw[i]) ) ), 2]
          history.redeem[, history.att.week[1]] <- history.redeem[, history.att.week[1]] + user.balance.data[  user.balance.data[,1]  %in% (date.range - (7*(week.max.withdraw[i])) ), 3]
      }
...
feature.week.month.group <- function(train.range, full.range, user.balance.data, interval.month = 15 ){
  user.balance.data <- user.balance.data[user.balance.data[, 1] %in% train.range,  ]
  ...
  for( week in  unique(train.week) ){
    for( mon in unique(train.month) ){
       ...
       purchase.value <- user.balance.data[ train.values.pos, 2 ]
       redeem.value <- user.balance.data[ train.values.pos, 3 ]
...
feature.enginearing1 <- function(train.date.range, predict.date.range, user.balance.data){
  ...
  month7.data <- feature.month(time.range, user.balance.data,  interval = 7 )
  holiday.data <- feature.holiday(time.range, need.before = T, need.after = T )
  interest.data <- feature.interest()
  # interest.data <- feature.interest.week(time.range )
  # history.data <- feature.history(time.range, user.balance.data  )
  week.month.data <- feature.week.month.group( train.date.range, time.range, user.balance.data )
...
#   
  ...
  train.purchase <- merge(feature.data[[1]][feature.data[[1]][,1] %in% train.date.range, ] , user.balance.data[,c(1, 2)], by='report_date')
  train.redeem <- merge(feature.data[[2]][feature.data[[2]][,1] %in% train.date.range, ] , user.balance.data[,c(1, 3)], by='report_date')
  ...
  ori.verify.data <- user.balance.data[user.balance.data[,1] %in% predict.date.range ,  ]
...
feature.enginearing2 <- function(preprocess.data, user.balance.data ){
  ...
  ### user.balance.data
  # user.balance.data <- get.user.balance.data()
  user.balance.data <- user.balance.data[user.balance.data[,1] %in% time.range, ]
  # browser()
  ...
  train.purchase$purchase <- user.balance.data[user.balance.data[ ,1]%in% train.time,  2]
  train.redeem$redeem <- user.balance.data[user.balance.data[ ,1]%in% train.time,  3]
  ...
  verify.user.balance.data <-  user.balance.data[user.balance.data[,1]%in% pred.time, ]
  # browser()
  return(list(train.purchase, train.redeem, pred.purchase, pred.redeem, verify.user.balance.data ) )
}
...
get.user.balance.data <- function(train.file ){
  user.balance.data <- read.csv(train.file)
  user.balance.data[,1] <- as.Date(as.character( user.balance.data[,1] ), format= '%Y%m%d')
  return(user.balance.data)
}

run.feature.engineering <- function(train.date.range, predict.date.range, train.file = 'data/p2_user_balance_sum.csv' ){
  user.balance.data <- get.user.balance.data(train.file) 
  # browser()
  feature.data1 <- feature.enginearing1(train.date.range, predict.date.range, user.balance.data)
  browser()
  feature.data2 <- feature.enginearing2( feature.data1, user.balance.data )
